...
function changeSOLD( $Kauf , $preis){
$output ='';
    if ($Kauf === NULL)
        {
            $output .=  "\n\t\t\t\t\t".'<span class="isIn">'. $preis .' &euro;</span>';
        }
        else 
        {
            $output .=  "\n\t\t\t\t\t".'<span class="isOut">SOLD</span>';
...
function getkuenstlerbotton(){
    ...
    if( isset($_SESSION['User']) && $_SESSION['User'] === 'isIN'){
        ...
        $button .= '<input type="submit" name="kunstupdate" value="Künstler werden" /></form>';
        $button .= '</form>'; 
    }
return $button;
}
...
function getBilduploadB ( $dbconn ){
    $output = '';
    $output .= '<form method="post" action="./index.php?page=KuenstlerStatus&'. SID .'">';
        $output .= '<input type="hidden" name="'. session_name() .' value="'.session_name() .'" />';    
        $output .= '<input type="submit" name="bildupload" value="Bild Hochladen" /></form>';
        $output .= '</form>'; 
return $output;
}
?>